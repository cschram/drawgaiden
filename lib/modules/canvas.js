    ...
    connect : function ( sock ) {
        ...
        sock.on( 'draw', function ( data ) {
            sock.get('name', function ( e, name ) {
                ...
                db.pushHistory( 'default', data ).fail(function ( err ) {
                    self.logger.error( 'Error saving canvas history: ' + err );
                });
            });
        });

        sock.on( 'canvas:history', function ( done ) {
            db.getHistory( 'default' ).then(function ( history ) {
                done( null, history );
            }, function ( err ) {
                self.logger.error( 'Error retrieving canvas history: ' + err );
                done( 'An unknown error occurred when retrieving the canvas history.' );
            });
        });
            ...
            sock.get('name', function ( e, name ) {
                ...
                db.getHistory( 'default' ).then(function ( history ) {
                    ...
                    db.clearUserHistory( 'default', name ).fail(function ( err ) {
                        self.logger.error( 'Error clearing user history: ' + err );
                    });
                    ...
                    self.logger.error( 'Error getting canvas history: ' + err );
        ...
        sock.on( 'clear-all', function () {
            db.clearHistory( 'default' ).fail(function ( err ) {
                self.logger.error( 'Error clearing canvas history: ' + err );
            });
